...
#e       = 1;
#p       = 1;
#trade   = (e*PFOR/p)^(1 / UPSILON);
#c       = (yss - kappav*vss - UPSILON * trade * CFOR)/(trade^UPSILON * (1 - UPSILON));
#NX      = (yss - trade^UPSILON*c - kappav*vss);
#NFAss   = NX/(1 - (1 + rss));
...
@#for name in [ "IM","HM"]
    ...
    1 + r_@{name} = exp(fic)*(1+R_@{name}(-1)+phi*(exp(NFA_@{name}-NFAss)-1))/(1+pi_@{name});

    // Eq. 2: Firm owner's SDF
    MF_@{name} = betaF*exp(-sig*(muz+fiz))*lamF_@{name}/lamF_@{name}(-1);

    // Eq. 3: Firm owner's MU of consumption
    lamF_@{name} = (cF_@{name}-hF*cF_@{name}(-1)*exp(-muz - fiz))^(-sig);
    ...
    lame_@{name} = (ce_@{name}-hF*ce_@{name}(-1)*exp(-muz - fiz))^(-sig);

    // Eq. 11: MU of consumption for people e->u
    lameu_@{name} = (ceu_@{name}-hF*ceu_@{name}(-1)*exp(-muz - fiz))^(-sig);
    ...
    // Eq. 35
    y_@{name}   = trade_@{name}^upsilon*(1-upsilon)*c_@{name} + upsilon*trade_@{name}*CFOR + kappav*v_@{name} ;

    // Eq. 36: 
    trade_@{name} = (e_@{name}*PFOR / p_@{name})^(1 / upsilon);

    // Eq. 37:
    (1 + R_@{name}) = (1 + RFOR)*e_@{name}(+1)/e_@{name};

     // 38
    NFA_@{name}(+1) = NX_@{name} + (1 + r_@{name})*NFA_@{name};

    // 39
    NX_@{name} = y_@{name} - trade_@{name}^upsilon*c_@{name} - kappav*v_@{name};
...
// --------------------------------------------------------------------
    ...
    NFA_IM = (1-Omega)*aF_IM + Omega*Ae_IM;
// --------------------------------------------------------------------
...
CFOR0         = 1;
e0_IM         = 1;
trade0_IM     = (e0_IM*PFOR0/p0_IM)^(1 / UPSILON);
piss0         = 0;
pi_star0      = 0;
Delta0_IM     = 1;
s0_IM         = csts/100;
...
NX0_IM        = (y0_IM - trade0_IM^UPSILON*c0_IM - kappav0*v0_IM);
NFA0_IM       = NX0_IM/(1 - (1 + r0_IM));
aF0_IM        = (NFA0_IM-Omega*Ae0_IM)/(1-Omega);
...
bu0_HM        = reprat*w0_HM;
tau0          = (1-n0_HM)*bu0_HM/(n0_HM*w0_HM);
cuu0_HM       = w0_HM*n0_HM;
ceu0_HM       = w0_HM*n0_HM;
ce0_HM        = w0_HM*n0_HM;
lame0_HM      = ( (1-hF/exp(muz0))*ce0_HM )^(-sig) ;
lameu0_HM     = ( (1-hF/exp(muz0))*ceu0_HM )^(-sig) ;
lamuu0_HM     = ( (1-hF/exp(muz0))*cuu0_HM )^(-sig) ;
e0_HM         = 1;
trade0_HM     = (e0_HM*PFOR0 / p0_HM)^(1 / upsilon);
c0_HM         = (y0_HM - kappav0*v0_HM - UPSILON * trade0_HM * CFOR0)/(trade0_HM^UPSILON * (1 - UPSILON));
cF0_HM        = (c0_HM - Omega*( n0_HM*ce0_HM + (1-f0_HM)*(1-n0_HM)*cuu0_HM + s0_HM*n0_HM*ceu0_HM ))/(1-Omega);
lamF0_HM      = ( (1-hF/exp(muz0))*cF0_HM )^(-sig) ;
x10_HM        = lamF0_HM*mc0_HM*y0_HM/(1-alpha*betaF0*(1+piss0)^theta);
x20_HM        = lamF0_HM*y0_HM/(1-alpha*betaF0*(1+piss0)^(theta-1));
NX0_HM        = (y0_HM - trade0_HM^UPSILON*c0_HM - kappav0*v0_HM);
NFA0_HM       = NX0_HM/(1 - (1 + r0_HM));
aF0_HM        = (NFA0_HM-Omega*Ae0_HM)/(1-Omega);
// --------------------------------------------------------------------
//
...
@#for name in [ "IM","HM"]
    ...
    pi_@{name}        = piss0;
    c_@{name}         = c0_@{name};
    trade_@{name}     = trade0_@{name};
    e_@{name}         = e0_@{name};
    p_@{name}         = p0_@{name};
    NFA_@{name}       = NFA0_@{name};
    NX_@{name}        = NX0_@{name};
@#endfor
